...
Primitives_t calcPrimitivesFromState(real8 state, real potentialEnergy) {
	...
	real magneticEnergyDensity = .5f * dot(prims.magneticField, prims.magneticField) / mhd_vacuumPermeability;
...
Wavespeed_t calcWavespeedFromPrimitives(Primitives_t prims) {
	...
	real starSpeedSq = .5f * (speedOfSoundSq + magneticFieldSq / (prims.density * mhd_vacuumPermeability));

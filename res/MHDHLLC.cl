...
internalEnergyDensityR = max(0.f, internalEnergyDensityR);	//magnetic energy is exceeding total energy ...
	...
	//HLLC-specific
	real qStar = (densityR * velocityR.x * (sr - velocityR.x) - densityL * velocityL.x * (sl - velocityL.x) + pressureL - pressureR - magneticFieldL.x * magneticFieldL.x - magneticFieldR.x * magneticFieldR.x) / (densityR * (sr - velocityR.x) - densityL * (sl - velocityL.x));
	if (0 <= sl) {
		for (int i = 0; i < NUM_STATES; ++i) {
			flux[i] = fluxL[i];
		}
		...
		magneticFieldStar.y = ((sl - velocityL.x - magneticFieldL.x * magneticFieldStar.x / (densityL * (sl - velocityL.x))) * magneticFieldL.y - (magneticFieldStar.x - magneticFieldL.x) * velocityL.y) / (sl - qStar - magneticFieldStar.x * magneticFieldStar.x / (densityL * (sl - velocityL.x)));
		magneticFieldStar.z = ((sl - velocityL.x - magneticFieldL.x * magneticFieldStar.x / (densityL * (sl - velocityL.x))) * magneticFieldL.z - (magneticFieldStar.x - magneticFieldL.x) * velocityL.z) / (sl - qStar - magneticFieldStar.x * magneticFieldStar.x / (densityL * (sl - velocityL.x)));
		magneticFieldStar.w = 0.f;
		real pressureStar = densityL * (sl - velocityL.x) * (qStar - velocityL.x) + pressureL + .5f * magneticFieldSqL - magneticFieldL.x * magneticFieldL.x + magneticFieldStar.x * magneticFieldStar.x;
		...
		stateLStar[STATE_MOMENTUM_Y] = (stateL[STATE_MOMENTUM_Y] * (sl - velocityL.x) - (magneticFieldStar.x * magneticFieldStar.y - magneticFieldL.x * magneticFieldL.y)) / (sl - qStar);
		stateLStar[STATE_MOMENTUM_Z] = (stateL[STATE_MOMENTUM_Z] * (sl - velocityL.x) - (magneticFieldStar.x * magneticFieldStar.z - magneticFieldL.x * magneticFieldL.z)) / (sl - qStar);
		...
		stateLStar[STATE_ENERGY_TOTAL] = (stateL[STATE_ENERGY_TOTAL] * (sl - velocityL.x) + (pressureStar * qStar - pressureL * velocityL.x) - (magneticFieldStar.x * vDotBStar - magneticFieldL.x * vDotBL)) / (sl - qStar);
		for (int i = 0; i < NUM_STATES; ++i) {
		...
		real4 magneticFieldStar;
		magneticFieldStar.x = (sr * magneticFieldR.x - sl * magneticFieldL.x) / (sr - sl);
		magneticFieldStar.y = ((sl - velocityR.x - magneticFieldR.x * magneticFieldStar.x / (densityR * (sl - velocityR.x))) * magneticFieldR.y - (magneticFieldStar.x - magneticFieldR.x) * velocityR.y) / (sl - qStar - magneticFieldStar.x * magneticFieldStar.x / (densityR * (sl - velocityR.x)));

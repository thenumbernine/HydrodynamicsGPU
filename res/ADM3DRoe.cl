...
D_kij = 1/2 partial_k gamma_ij <=> 2 D_kij = partial_k gamma_ij
V_k = D_km^m - D^m_mk
...
partial_gamma_mn gamma^ij 
= derivative of an inverse wrt its matrix
= -g^im gamma^nj

partial_k gamma^ij
= partial_gamma_mn gamma^ij partial_k gamma_mn
= -g^im gamma^nj partial_k gamma_mn

partial_k tr K
= partial_k (K_ij gamma^ij)
= gamma^ij partial_k K_ij + K_ij partial_k gamma^ij
= gamma^ij partial_k K_ij + K_ij partial_gamma_mn gamma^ij partial_k gamma_mn
= gamma^ij partial_k K_ij - K_ij gamma^im gamma^nj partial_k gamma_mn
...
= partial_k (gamma^km D_mij) + 1/2 delta^k_i (partial_k A_j + 2 partial_k V_j - partial_k (D_jmn gamma^mn)) + 1/2 delta^k_j (partial_k A_i + 2 partial_k V_i - partial_k (D_imn gamma^mn))
= D_mij partial_k gamma^km + gamma^km partial_k D_mij 
	+ 1/2 (delta^k_i partial_k A_j + delta^k_j partial_k A_i)
	+ delta^k_i partial_k V_j + delta^k_j partial_k V_i
= -D_mij gamma^kr gamma^sm partial_k gamma_rs + gamma^km partial_k D_mij
	+ 1/2 (delta^k_i partial_k A_j + delta^k_j partial_k A_i)
	+ delta^k_i partial_k V_j + delta^k_j partial_k V_i
= -2 D_mij D_k^km + gamma^km partial_k D_mij
	+ 1/2 (delta^k_i partial_k A_j + delta^k_j partial_k A_i)
	+ delta^k_i partial_k V_j + delta^k_j partial_k V_i
...
... partial_t K_ij + alpha (-2 D_mij D_k^km + gamma^km partial_k D_mij
	+ 1/2 (delta^k_i partial_k A_j + delta^k_j partial_k A_i)
	+ delta^k_i partial_k V_j + delta^k_j partial_k V_i) 
	= alpha S_ij - alpha lambda^k_ij A_k
... partial_t K_ij + alpha (gamma^km partial_k D_mij
	+ 1/2 (delta^k_i partial_k A_j + delta^k_j partial_k A_i)
	+ delta^k_i partial_k V_j + delta^k_j partial_k V_i) 
...
check out the eigenfields for how to get the left eigenvectors
